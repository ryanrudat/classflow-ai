 ...
 * Persistent top bar for authenticated teachers.
 */
export default function Navigation() {
  ...
  // Hide on login/register and on every student-facing route
  const hideNavRoutes = ['/login', '/register']
  const isStudentRoute =
    location.pathname.startsWith('/join') ||
    location.pathname.startsWith('/speak/') ||
    (location.pathname.startsWith('/reverse-tutoring/') && !location.pathname.startsWith('/reverse-tutoring/dashboard'))

  if (!user || hideNavRoutes.includes(location.pathname) || isStudentRoute) {
    return null
  }
  ...
  const linkClass = isDashboard
    ? 'px-4 py-2 rounded-lg font-medium bg-blue-600 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2'
    : 'px-4 py-2 rounded-lg font-medium text-gray-700 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2'
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-6">
              ...
              <Link to="/dashboard" className={linkClass} aria-current={isDashboard ? 'page' : undefined}>
                Sessions
              </Link>
            </div>
